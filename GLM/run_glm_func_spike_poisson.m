function run_glm_func_spike_poisson(SESS_NAME, MOUSE, SESSION)
...
%    makeFigure(spike_rate_fit_long,spike_rate_fit_test_long,coef_long,spike_rate_test_long,spike_rate_pred_long);
    ...
    save_loc = strcat(loc_info.raw_dir(2:end-1), MOUSE, '\', SESSION, '\data_glm_fit_spike\',int2str(ROI),'.mat');
    if ROI == 1
        status = mkdir(strcat(loc_info.raw_dir(2:end-1), MOUSE, '\', SESSION, '\data_glm_fit_spike'));
    end
