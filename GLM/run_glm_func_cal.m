function run_glm_func_cal(SESS_NAME, MOUSE, SESSION)
...
lmcenter_pred_short = fitDistLmcenter(behav_ds, 3);
predictor_short_x_0 = makeSpeedPred(behav_ds,lmcenter_pred_short, 3);
...
for ROI=1:ROI_amount
    ...
    % apply glmnet
    fit_short = glmnet(predictor_short_x_fit,spike_rate_fit_short, 'gaussian', options);
...
%    makeFigure(spike_rate_fit_short,spike_rate_fit_test_short,coef_short,spike_rate_test_short,spike_rate_pred_short);
    ...
    fit_long = glmnet(predictor_long_x_fit,spike_rate_fit_long, 'gaussian', options);
...
%    makeFigure(spike_rate_fit_long,spike_rate_fit_test_long,coef_long,spike_rate_test_long,spike_rate_pred_long);
    ...
    save_loc = strcat(loc_info.raw_dir(2:end-1), MOUSE, '\', SESSION, '\data_glm_fit_cal\',int2str(ROI),'.mat');
    if ROI == 1
        status = mkdir(strcat(loc_info.raw_dir(2:end-1), MOUSE, '\', SESSION, '\data_glm_fit_cal'));
    end
