...
cellreg_data = load('D:\Lukas\extra\roi results\cellRegistered_LF191023_blue_13_08_reg.mat');
...
%for i = 1:1
    ...
    long_mat = [];
    for x = 1:numb
        file = strcat(loc_info.raw_dir(2:end-1), MOUSE, '\', SESSION(x,:), '\glm_fits_calcium_gaussian\',int2str(comb_mat(i,x)),'.mat');
        data = load(file);
        if x == 1
