...
# number of columns to be imported from the raw data
num_columns = 11
...
datafilenames = filedialog.askopenfilenames(initialdir='/Users/lukasfischer/Work/exps/MTH3/opto/vr2/opto rig raw', title='Please pick DATAFILES')
root.update()
if not datafilenames:
    raise NameError('Please select datafiles.')

configfilename = filedialog.askopenfilename(initialdir='/Users/lukasfischer/Google Drive/MTH3_data/config_files', title='Please pick CONFIG FILE')
root.update()
if not configfilename:
    warnings.warn("No config file selected", UserWarning)

h5filename = filedialog.askopenfilenames(initialdir='/Users/lukasfischer/Google Drive/MTH3_data/animals_h5', title='Please pick HDF5-FILE')
root.update()
if not h5filename:
...
for row in datafilenames:
    ...
    if day.find('opto') != -1:
        # folder_suffix = '_s5r'
        day = day.replace('_opto', '')
        print('opto session detected')
    if day.find('s2r_defrew') != -1:
        # folder_suffix = '_s5r'
        day = day.replace('_s2r_defrew', '')
        print('s2r_defrew session detected')
    ...
    datcont = np.ndarray(shape=(row_count, num_columns), dtype=float)
    ...
    for line in raw_data:
        ...
        # check if extra columns exist (which will contain opto mask and opto stim info)
        if len(line) > 9:
            datcont[i - skiplines][9] = line[9]
            datcont[i - skiplines][10] = line[10]
    ...
    tarr.resize((row_count - skiplines, num_columns))
    # read config file
    ...
    if multilayer_suffix is not None:
        for osuff in multilayer_suffix:
            ...
            print(np.size(tarr,0),np.size(tarr,1))
            try:
            ...
            write_h5(f, day+folder_suffix+'_'+osuff, rewards(raw), 'rewards')
            rewards_ds = np.copy(f['Day' + str( day+folder_suffix+'_'+osuff) + '/rewards'])

            lick_pre, licks_post = licks(raw_filt, rewards_ds)
            write_h5(f, day+folder_suffix+'_'+osuff, lick_pre, 'licks_pre_reward')
            write_h5(f, day+folder_suffix+'_'+osuff, licks_post, 'licks_post_rewar')
    else:
        # retrieve group identifier, if group doesn't exist, create it
        if optional_suffix is not None:
            folder_suffix =  folder_suffix + '_' + optional_suffix[0]
        ...
        raw = np.copy(f['Day' + str(day) + folder_suffix + '/raw_data'])
        # remove times when mouse was in the black box
        no_blackbox_trials = filter_trials(raw, [], ['tracknumber', track])
        no_blackbox_trials = np.union1d(
            no_blackbox_trials, filter_trials(raw, [], ['tracknumber', track_long]))
        ...
        write_h5(f, day+folder_suffix, rewards(raw), 'rewards')
        rewards_ds = np.copy(f['Day' + str( day+folder_suffix) + '/rewards'])

        lick_pre, licks_post = licks(raw_filt, rewards_ds)
        write_h5(f, day+folder_suffix, lick_pre, 'licks_pre_reward')
        write_h5(f, day+folder_suffix, licks_post, 'licks_post_rewar')
