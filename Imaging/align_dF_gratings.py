...
def align_dF_gratings( raw_behaviour, raw_dF, frame_bri=[], behav_idx_range=[-1,-1], gcamp_idx_skip=[-1,-1], interp=True, tt_reject=True, session_crop=[0,1] ): #for current frame latency issue end point should be 1790.15s (based on estimate of 40.22Hz sampling)
    ...
    if behav_idx_range[0] != -1:
        behaviour_aligned = behaviour_aligned[behav_idx_range[0]:,:]
    if behav_idx_range[1] != -1:
        behaviour_aligned = behaviour_aligned[:(np.size(behaviour_aligned,0)-behav_idx_range[1]),:]
    ...
    if gcamp_idx_range[1] != -1:
        dF_aligned = dF_aligned[:(np.size(raw_dF,0)-gcamp_idx_range[1]),:]
...
#        behaviour_aligned = np.zeros((np.size(raw_behaviour,0),np.size(raw_behaviour,1)))
        ...
        behaviour_aligned[:,5] = np.insert(np.diff(even_ts),np.mean(even_ts),0)
        ...
        for i,ats in enumerate(raw_behaviour[:,0]):
            ...
            if behaviour_aligned[new_idx[i],8] == 0:
                behaviour_aligned[new_idx[i],8] = raw_behaviour[i,8]

            new_idx[i] = (np.abs(behaviour_aligned[:,0]-ats)).argmin()
            behaviour_aligned[new_idx[i],1] = raw_behaviour[i,1]
        ...
        for i in range(1,np.size(new_trial_idx,0)):
            behaviour_aligned[new_trial_idx[i-1]+1:new_trial_idx[i]+1,1] = trial_tracks[i-1]
