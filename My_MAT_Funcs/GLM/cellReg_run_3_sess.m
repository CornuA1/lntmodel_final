...
function cellReg_run_3_sess(mouse,status,session1, session2, session3) 
% Defining the results_directory and creating the figures_directory:
results_directory=strcat('D:\Lukas\data\animals_raw\',mouse,'\results_',status);
figures_directory=fullfile(results_directory,'Figures');
if exist(figures_directory,'dir')~=7
    mkdir(figures_directory);
end
figures_visibility='off'; % either 'on' or 'off' (in any case figures are saved)prunned_cell_bodies
save_file_name = strcat(mouse,'_',status);
file_names={strcat('D:\Lukas\data\animals_raw\',mouse,'\',session1,'\adjusted_cell_bodies.mat') ,...
            strcat('D:\Lukas\data\animals_raw\',mouse,'\',session2,'\adjusted_cell_bodies.mat') ,...
            strcat('D:\Lukas\data\animals_raw\',mouse,'\',session3,'\prunned_cell_bodies.mat')};
...
use_parallel_processing=false; % either true or false
...
sufficient_correlation_centroids=0.8; % smaller correlation imply no similarity between sessions
sufficient_correlation_footprints=0.4; % smaller correlation imply no similarity between sessions
...
maximal_distance=10; % cell-pairs that are more than 12 micrometers apart are assumed to be different cells
...
p_same_threshold=0.85; % only relevant if probabilistic approach is used
...
save(strcat(results_directory,'\cellRegistered_',save_file_name,'.mat'),'cell_registered_struct')
