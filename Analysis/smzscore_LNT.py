...
from rewards import rewards as extract_rewards
from licks import licks as extract_licks


def smzscore( raw, rz, default, shuffles, tracklength, mode=-1 ):
    """ Calculate the SMZ (Spatial z-score) for the provided session """

    # get rewards and licks
    rewards_all = extract_rewards(raw)
    raw_filt = raw[np.in1d(raw[:, 4], [3,4]), :]
    licks_all = extract_licks(raw_filt, rewards_all)[0]

    # filter licks and rewards for current trial type
    rewards = select_trials(rewards_all, 'reward', mode)
    licks = select_trials(licks_all, 'licks', mode)

    success_rate = (rewards[rewards[:,5]==1,5].shape[0]) / rewards.shape[0]
