...
Each simulated FOV dataset contains a given distribution of a different types
of neurons.
...
def create_sim_cell(behav_ds, cell_type, place_cell_loc, S, E, fname, cell_index):
    ...
    time_vector_upsampled = np.interp(np.arange(0,time_vector[-1],1/samplingRate),np.arange(0,time_vector[-1],1/orig_samplingRate),time_vector)
    trialtype_vector_upsampled = np.interp(np.arange(0,time_vector[-1],1/samplingRate),np.arange(0,time_vector[-1],1/orig_samplingRate),behav_ds[:,4])
    speed_filtered_upsampled = np.interp(np.arange(0,time_vector[-1],1/samplingRate),np.arange(0,time_vector[-1],1/orig_samplingRate),speed_filtered)
...
if __name__ == "__main__":
    ...
    if sim_cells == True:
        ...
        cell_types = {
            ...
            # 'track_point' : [[220,-40],[220,-35],[220,-30],[220,-25],[220,-20],[220,-15],[220,-10],[220,-5],[220,0],[220,5],[220,10],[220,15],[220,20],[220,30],[220,40],[220,50],[220,60],[220,70],[220,80]]
        ...
        write_dict(sim_mouse, sim_day, roi_result_params, False, True, 'jitter')
        write_dict(sim_mouse, sim_day, roi_result_params, False, True, 'wide_RF')



        # set up empty dicts and fill them
        cell_types_vr = {'track_point':[]}
        cell_types_jitter = {'track_point':[]}
        cell_types_wide_RF = {'track_point':[]}
        for i in range(n):
            RF_center = np.random.uniform(-50,50)
            cell_types_vr['track_point'].append([220,RF_center,E,'lm'+str(i),i])
            cell_types_jitter['track_point'].append([220,RF_center,E_jitter,'lmj'+str(i),i])
            cell_types_wide_RF['track_point'].append([220,RF_center,E_wide_RF,'lmwrf'+str(i),i])

        # simulate dF/F traces
        dffs_vr = sim_FOV(behav_ds, cell_types_vr, S)
        dffs_jitter = sim_FOV(behav_ds, cell_types_jitter, S)
        dffs_wide_RF = sim_FOV(behav_ds, cell_types_wide_RF, S)
        ...
        sio.savemat(loc_info['raw_dir'] + sim_mouse + os.sep + sim_day+jitter_suffix + os.sep + 'aligned_data.mat', mdict={'dF_aligned' : dffs_jitter, 'behaviour_aligned' : behav_ds})
        sio.savemat(loc_info['raw_dir'] + sim_mouse + os.sep + sim_day+wide_RF_suffix + os.sep + 'aligned_data.mat', mdict={'dF_aligned' : dffs_wide_RF, 'behaviour_aligned' : behav_ds})
