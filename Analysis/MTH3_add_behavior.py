...
print('multilayer suffix: ' + str(multilayer_suffix))
print('optional suffix: ' + str(optional_suffix))
...
for row in datafilenames:
    ...
    if day.find('s5r2') != -1:
        # folder_suffix = '_s5r'
        day = day.replace('_s5r2', '')
        print('s5r2 session detected')
    elif day.find('s5r') != -1:
        # folder_suffix = '_s5r'
        day = day.replace('_s5r', '')
        print('s5r session detected')
    if day.find('openloop') != -1:
        folder_suffix = '_openloop'
        day = day.replace('_openloop', '')
        print('openloop session detected')
    if day.find('dark') != -1:
        folder_suffix = '_dark'
    ...
    for line in raw_data:
        ...
        # older datasets will have fewer columns
        if len(line) > 8:
    ...
    if multilayer_suffix is not None:
        for osuff in multilayer_suffix:
            ...
            if optional_suffix is not None:
                folder_suffix_temp =  folder_suffix + '_' + optional_suffix[0]
            else:
                folder_suffix_temp =  folder_suffix
            # retrieve group identifier, if group doesn't exist, create it
            daygrp = f.require_group('Day' + day + folder_suffix_temp + '_' + osuff)
            ...
            raw = np.copy(f['Day' + str(day) + folder_suffix_temp + '_' + osuff + '/raw_data'])
            ...
            write_h5(f, day+folder_suffix_temp+'_'+osuff, rewards(raw), 'rewards')
            rewards_ds = np.copy(f['Day' + str( day+folder_suffix_temp+'_'+osuff) + '/rewards'])

            lick_pre, licks_post = licks(raw_filt, rewards_ds)
            write_h5(f, day+folder_suffix_temp+'_'+osuff, lick_pre, 'licks_pre_reward')
            write_h5(f, day+folder_suffix_temp+'_'+osuff, licks_post, 'licks_post_rewar')
    else:
        # retrieve group identifier, if group doesn't exist, create it
        if optional_suffix is not None:
