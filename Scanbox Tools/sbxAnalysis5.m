...
%     colormap(GUIHandles.roiDrawing.imageAxes, gray);
    ...
    GUIHandles.roiDrawing.frameSlider = uicontrol('Parent', GUIHandles.roiDrawingTab, 'Style', 'slider', 'Min', 0, 'Max', GUIHandles.Info.maxIndex, 'Value', 0, 'SliderStep', [1/GUIHandles.Info.maxIndex, 100/GUIHandles.Info.maxIndex], 'Interruptible', 'on', 'Visible', 'off', 'Units', 'normalized', 'Position', [0.0075, GUIHandles.roiDrawing.imageTypeButtonGroup.Position(2) - 0.0175, 0.085, 0.015], 'Callback', @frameSliderCallback);
...
%         set(0, 'CurrentFigure', temp_fig)
        ...
        normcorreCorrection = true;
        if normcorreCorrection
            disp('Preparing Normcorre analysis...');
            N = GUIHandles.Info.maxIndex;
            %Y_n = sbxgrabframe(strcat(GUIHandles.sbxPath,GUIHandles.sbxName),0,-1); % GUIHandles.tifData;
%             Y_n = sbxreadframes(GUIHandles.sbxName,0,N,Parameters);
%             Y = squeeze(Y_n);
%             Y = single(Y);                 % convert to single precision 
%             T = size(Y,ndims(Y));          % total number of frames
%             min_val = min(Y(:));
%             Y = Y - min_val;             % remove baseline from absolute values

            % now try non-rigid motion correction (also in parallel)
            disp('Running Normcorre analysis...');
%             options_nonrigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'grid_size',[32,32],'mot_uf',4,'bin_width',200,'max_shift',15,'max_dev',3,'us_fac',50,'init_batch',200);
            % tic; [M2,shifts2,template2,options_nonrigid] = normcorre_batch(Y,options_nonrigid); toc
            % to do: split into batches of frames, remember to take
            % template from first batch
            full_batch = true;
            batch_n = 1;            % batch number
            batch_frames = 3000;      % frames per batch
%             total_num_batches = idivide(T, batch_frames) + 1;
            while full_batch
                start_frame = (batch_n-1)*batch_frames;
                disp('Reading')
                try
                    Y_n = sbxreadframes(GUIHandles.sbxName,start_frame,batch_frames,Parameters);
%                     Y_n = sbxgrabframe(strcat(GUIHandles.sbxPath,GUIHandles.sbxName),start_frame,batch_frames);

%                     Y_batch = Y(:,:,double((batch_n-1)*batch_frames + 1):batch_n*batch_frames);
                catch
                    full_batch = false;
                    Y_n = sbxreadframes(GUIHandles.sbxName,start_frame,N-start_frame+1,Parameters);
%                     Y_n =
%                     sbxgrabframe(strcat(GUIHandles.sbxPath,GUIHandles.sbxName),start_frame,N-start_frame+1);
%                     empty last frame, looks like more noise; if remove +1
%                     -static
%                     Y_batch = Y(:,:,double((batch_n-1)*batch_frames + 1):end);
                ...
                if batch_n == 1
                    disp('First')
                    % run normcorre, save template
                    options_nonrigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'grid_size',[32,32],'mot_uf',4,'bin_width',200,'max_shift',15,'max_dev',3,'us_fac',50,'init_batch',200);
                    [M2,~, template2,~] = normcorre_batch(Y,options_nonrigid);
                else
                    disp('Second')
                    % run normcorre with given template
                    [Mbatch,~,~,~] = normcorre_batch(Y,options_nonrigid, template2);
                    disp('Concatenate')
                    % concantenate Mbatch with M2
                    M2 = cat(3, M2, Mbatch);
            ...
            disp('Saving motion corrected data...');
            newFileID = fopen([GUIHandles.sbxName, '_rigid.sbx'], 'w');
            %min_val = min(Y(:));
            GUIHandles.Info.samplesPerFrame = size(M2,1) * size(M2,2);
            GUIHandles.Info.sz(1) = size(M2,1);
            GUIHandles.Info.sz(2) = size(M2,2);
            for n=1:N  
                frame = reshape(permute(M2(:,:,n),[2,1]),[GUIHandles.Info.samplesPerFrame,1]);
                %frame = frame + min_val;
            ...
            fclose(newFileID);
            %static:
            %save([GUIHandles.Info.Directory.folder, GUIHandles.Info.Directory.name, '_rigid.sbx'], 'M2', '-v7.3');
            %error:
            %save([GUIHandles.sbxName, '_rigid.mat'], 'GUIHandles.Info');
            try
                % save([GUIHandles.sbxName, '_rigid.mat'], 'GUIHandles', '-v7.3');
                copyfile([GUIHandles.sbxName, '.mat'], [GUIHandles.sbxName, '_rigid.mat'])
            catch
                disp('yeehaw it didnt work again');
            end
            ...
            columnShifts = zeros(1,GUIHandles.Info.maxIndex+1);
            frameCrop = [0,0,0,0];
            save([GUIHandles.Info.Directory.folder, GUIHandles.Info.Directory.name, '_rigid.rigid'], 'phaseDifferences', 'rowShifts', 'columnShifts', 'frameCrop');
            GUIHandles.Info.Directory.name = char([GUIHandles.Info.Directory.name, '_rigid']);
...
function computeCallback(GUI, ~)
    ...
    Parameters.cc_choice = questdlg('Calculate cross correlation maps?', 'CC calculation', 'Yes', 'No','No');
