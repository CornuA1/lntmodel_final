function zstack_hackedpreanalysis(indexyo)
    ...
    GUI = figure('Name', 'Scanbox Analysis', 'NumberTitle', 'off', 'MenuBar', 'none', 'Color', 'w', 'Units', 'normalized', 'Position', [0.125, 0.125, 0.75, 0.75]);
    ...
    GUIHandles.loadSBXButton = uicontrol('Style', 'pushbutton', 'String', 'Load Imaging Data', 'Units', 'normalized', 'Position', [0.013, 0.92, 0.074, 0.05], 'Callback', @loadSBXCallback);
...
%     answer = inputdlg({'Min frame:', 'Max frame:'}, 'Frames to sample from for image stack', 1, {'0', int2str(GUIHandles.Info.maxIndex)});
    ...
    Parameters.sampleBounds(2) = GUIHandles.Info.maxIndex;
    
    Parameters.maxQuantile = 0.95;
    
    GUIHandles.imageProcessing.computeButton.Visible = 'off';
    
    [meanImage, maxIntensityImage, primage50, primage75, primage90,result] = sbxPreAnalysis_lou2(GUIHandles.Info, Parameters);
disp('done bitch')
...
% [Info.Directory.folder, idx, 'compiledstack'];
sbxName=[indexyo,'compiledstack']
sbxPath='F:\Quique\pizzaman stack\pizzaman4\'
...
%     colormap(GUIHandles.roiDrawing.imageAxes, gray);
    ...
    GUIHandles.roiDrawing.frameSlider = uicontrol('Parent', GUIHandles.roiDrawingTab, 'Style', 'slider', 'Min', 0, 'Max', GUIHandles.Info.maxIndex, 'Value', 0, 'SliderStep', [1/GUIHandles.Info.maxIndex, 50/GUIHandles.Info.maxIndex], 'Interruptible', 'on', 'Visible', 'off', 'Units', 'normalized', 'Position', [0.0075, GUIHandles.roiDrawing.imageTypeButtonGroup.Position(2) - 0.0175, 0.085, 0.015], 'Callback', @frameSliderCallback);
    ...
    GUIHandles.roiDrawing.hideROIsButton = uicontrol('Parent', GUIHandles.roiDrawingTab, 'Style', 'checkbox', 'String', 'Hide ROIs', 'BackgroundColor', 'w', 'Units', 'normalized', 'Position', [0.925, 0.525, 0.05, 0.025], 'Callback', @hideROIsCallback);
    
    GUIHandles.roiDrawing.ccLocalButton = uicontrol('Parent', GUIHandles.roiDrawingTab, 'Style', 'checkbox', 'String', 'Local Cross-Correlation', 'BackgroundColor', 'w', 'Units', 'normalized', 'Position', [0.9025, 0.5, 0.095, 0.025], 'Callback', @ccLocalCallback);

    GUIHandles.roiDrawing.manipulateROIsButtonGroup = uibuttongroup('Parent', GUIHandles.roiDrawingTab, 'Title', 'ROI Manipulation', 'TitlePosition', 'centertop', 'BackgroundColor', 'w', 'Units', 'normalized', 'Position', [0.9125, 0.395, 0.075, 0.0849], 'SelectionChangedFcn', @manipulateROIsButtonGroupSelectionFunction);
    ...
    GUIHandles.roiDrawing.drawShapeButtonGroup = uibuttongroup('Parent', GUIHandles.roiDrawingTab, 'Title', 'ROI Draw Method', 'TitlePosition', 'centertop', 'BackgroundColor', 'w', 'Units', 'normalized', 'Position', [0.9125, 0.2567, 0.075, 0.1283], 'SelectionChangedFcn', @drawTypeButtonGroupSelectionFunction);
...
function loadNewSBXCallback(GUI, ~)
    ...
    if exist([sbxPath, sbxName, '.rigid'], 'file')
        ...
        GUIHandles.motionCorrected = true;
    elseif exist([sbxPath, sbxName, '.'], 'file')
        load([sbxPath, sbxName, '.align'], '-mat', 'phaseDifferences', 'rowShifts', 'columnShifts', 'frameCrop');
...
function playCallback(GUI, ~)
    ...
    for f = 0:15:GUIHandles.Info.maxIndex
        GUIHandles = guidata(GUI);
...
function resetCallback(GUI, ~)
    ...
    GUIHandles.imageProcessing.resonantOffsetSlider.Value = GUIHandles.defaultResonantOffset;
    GUIHandles.roiDrawing.resonantOffsetSlider.Value = GUIHandles.defaultResonantOffset;
    
    GUIHandles.imageProcessing.brightnessSlider.Value = GUIHandles.defaultBrightness;
    GUIHandles.roiDrawing.brightnessSlider.Value = GUIHandles.defaultBrightness;
    
    GUIHandles.imageProcessing.contrastSlider.Value = GUIHandles.defaultContrast;
    GUIHandles.roiDrawing.contrastSlider.Value = GUIHandles.defaultContrast;
    
    guidata(GUI, GUIHandles);
    
    resonantOffsetSliderCallback(GUI);
    brightnessSliderCallback(GUI);
    contrastSliderCallback(GUI);
...
if ~GUIHandles.motionCorrected
    choice = questdlg('Crop imaging data before motion correction?', 'Crop Frame', 'Yes', 'No','Lou way', 'Yes');
...
function motionCorrectCallback(GUI, ~)
    ...
    choice = questdlg('Crop imaging data before motion correction?', 'Crop Frame', 'Yes', 'No', 'No');
...
function extractFluorescencesCallback(GUI, ~)
    ...
    if nnz(GUIHandles.roiDrawing.roiMask) > 0
        ...
        progressBar = waitbar(0, 'Extracting fluorescences...', 'Name', [GUIHandles.sbxName, ': Extracting Fluorescences'], 'CreateCancelBtn', 'setappdata(gcbf, ''Canceling'', 1)');
        setappdata(progressBar, 'Canceling', 0);
        ...
        if isempty(lightMask)
            save([fileName, '.extra'], 'version', 'meanBrightness', 'roiCoordinates', 'roiSizes');
        else
            save([fileName, '.extra'], 'version', 'lightContaminations', 'meanBrightness', 'roiCoordinates', 'roiSizes');
        end
...
function deleteROIsCallback(GUI, ~)
    ...
    if GUIHandles.roiDrawing.selectedROI(1) > 0
        if ~strcmp(GUIHandles.roiDrawing.manipulateType, 'Draw')
            ...
            for r = length(GUIHandles.roiDrawing.selectedROI):-1:1
                GUIHandles.roiDrawing.previousROIMask(GUIHandles.roiDrawing.selectedROIMask == GUIHandles.roiDrawing.selectedROI(r)) = GUIHandles.roiDrawing.selectedROI(r);
...
function saveROIsCallback(GUI, ~)
    ...
    save([fileName, '.rois'], 'roiMask', 'overlapMask', 'vertices');
        ...
        case 'Image Processing'
            switch GUIHandles.imageProcessing.imageType
                ...
                case 'Frame-by-Frame'
                    ...
                    GUIHandles.imageProcessing.displayedImage.CData = double(frame)/double(intmax('uint16'));
            end
