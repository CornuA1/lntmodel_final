...
%     colormap(GUIHandles.roiDrawing.imageAxes, gray);
    ...
    GUIHandles.roiDrawing.roiTreeLevel.String = strcat('Parent: ', num2str(GUIHandles.roiDrawing.roiTreeCurrentParent{1}));
...
function extractFluorescencesCallback(GUI, ~)
    ...
    if nnz(GUIHandles.roiDrawing.roiMask) > 0
        ...
        if isempty(lightMask)
            ...
            save([fileName, '.extra'], 'version', 'lightContaminations', 'meanBrightness', 'roiCoordinates', 'roiSizes', 'roiIDs');
        end
...
function candidateROIImageClickFunction(GUI, ~)
    ...
    if isempty(GUIHandles.roiDrawing.roiTree.String)
        newROIID = 1;
        GUIHandles.roiDrawing.roiTree.String{end+1} = num2str(newROIID);
    else
...
function lastroi = find_last_in_roiTree(roiTree, parentROI)
    ...
    for j=1:length(roiTree.String) % loop through all entries
        ...
        if parentIndexLevel == 0 % special case on 0-level where we just want to coun the number of length=1 roi entries
            if length(roiTree_cur_id) == 1
                num_in_parent = num_in_parent + 1;
            end
        else
            if length(roiTree_cur_id) == parentIndexLevel+1 % check if current line is on the desired insertion level (n.b. this is before we check if its the right parent)
                if strcmp(roiTree_cur_id(1:parentIndexLevel), parentROI{1})
                    num_in_parent = num_in_parent + 1;
                end
            end
        end
    ...
    if strcmp(parentROI{1}, '0')
        lastroi = num_in_parent;
    else
        lastroi = parentROI{1};
        lastroi(end+1) = num2str(num_in_parent);
...
function tree_content = order_roi_tree(roiTree)
    ...
    tree_content = cellfun(@str2num, roiTree.String);
   
    max_level = 0;
    for i=1:length(tree_content) % get max depth of the tree
        if numel(num2str(tree_content(i))) > max_level
            max_level = numel(num2str(tree_content(i)));
        end
    end
    
    for i=1:length(tree_content) % multiply value on upper levels so that 
        tree_content(i) = tree_content(i) * (10^(max_level - numel(num2str(tree_content(i)))));
    end
    
    tree_content = sort(tree_content);
    
    for i=1:length(tree_content) % multiply value on upper levels so that
        while mod(tree_content(i),10) == 0
            tree_content(i) = tree_content(i)/10;
        end
    end
    
    tree_content = num2cell(num2str(tree_content),2);
