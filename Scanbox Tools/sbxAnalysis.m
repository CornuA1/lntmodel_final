...
function motionCorrectCallback(GUI, ~)
    ...
    if strfind(GUIHandles.sbxName, '_rigid') > 0
        dummyalign = questdlg('Create a dummy .align file?', 'Dummy align', 'Yes', 'No','Yes');
    end

    if strcmp(dummyalign, 'Yes') % create a dummy file for when the file has already been motion corrected with a different algorithm
        ...
        frameCrop = [0,0,0,0];
        save([GUIHandles.Info.Directory.folder, GUIHandles.Info.Directory.name, '.rigid'], 'phaseDifferences', 'rowShifts', 'columnShifts', 'frameCrop');
        GUIHandles.imageProcessing.motionCorrectButton.Visible = 'off';
        result = 'Completed';
    else
...
function saveROIsCallback(GUI, ~)
    ...
    if exist([fileName, '.rois'], 'file')
        ...
        if strcmp(choice, 'No')
            ...
            while exist([GUIHandles.Info.Directory.name, '_', int2str(number), '.rois'], 'file')
                number = number + 1;
            end

            fileName = [GUIHandles.Info.Directory.name, '_', int2str(number)];
            
            if ~exist([GUIHandles.Info.Directory.name, '.rois'], 'file')
                fileName = [GUIHandles.Info.Directory.name];
            end
        end
    end
