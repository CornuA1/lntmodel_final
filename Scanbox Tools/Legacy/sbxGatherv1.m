function result = sbxGather(Info, Parameters)
    ...
    if ~exist([Info.Directory.folder, Info.Directory.name, '.align'], 'file')
    ...
    for p = 1:nPlanes
        ...
        for f = 1:length(framesToGather)
            registeredFrames(f, :, :) = applyMotionCorrection(Info, f, framesToGather, frameCrop, phaseDifferences, rowShifts, columnShifts);
        end
...
function adjustedImage = applyMotionCorrection(Info, f, framesToAlign, frameCrop, phaseDifferences, rowShifts, columnShifts)
    ...
    phaseDifference = phaseDifferences(framesToAlign(f) + 1);
    rowShift = rowShifts(framesToAlign(f) + 1);
    columnShift = columnShifts(framesToAlign(f) + 1);
