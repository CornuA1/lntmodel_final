...
""
do Factor Analysis for short and long trials based on the mean dF signal of the ROIs
...
def Factor_Analysis_mean_trials(plt, short_dF_temp, long_dF_temp,transformer_long_temp, transformer_short_temp):
    import numpy as np
    ...
    #standardize dF by all ROIs (required for Factor_Analysis analysis) 
...
def Factor_Analysis_all_trials(ax,all_trials_short,all_trials_long,trials_short_size,trials_long_size, count, transformer_long_temp, transformer_short_temp):
    import numpy as np
    import warnings; warnings.simplefilter('ignore')
    import sys
    sys.path.append("../Analysis")
...
if __name__ == "__main__":
    ...
    for i in range(len(good_recordings)):
        ...
        tf_temp = FactorAnalysis(n_components=k, random_state=0)
        transformer_short_temp = tf_temp.fit(mean_trials_short)
        transformer_long_temp = tf_temp.fit(mean_trials_long)
      
        Factor_Analysis_all_trials(plt,np.asarray(all_trials_short),np.asarray(all_trials_long),trials_short_size[0],trials_long_size[0], count,transformer_long_temp, transformer_short_temp)
        plt.xlabel('PC1')
        plt.ylabel('PC2')
        plt.title('%s' % mouse)
        ...
        transformer_short_temp = tf_temp.fit(mean_trials_short)
        transformer_long_temp = tf_temp.fit(mean_trials_long)
        Factor_Analysis_mean_trials(plt, mean_trials_short, mean_trials_long,transformer_long_temp, transformer_short_temp)
        plt.xlabel('PC1')
        plt.ylabel('PC2')
        plt.title('%s' % mouse)
        count = count + 2
    
    subfolder = 'FA_dtw_figures'
    fformat = 'png'
    ffname = 'FA_analysis_all_trials_withmean'
