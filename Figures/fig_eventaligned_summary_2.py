...
def event_maxresponse(roi_param_list_all, event_list, trialtypes, peak_metric, ax_object, ax_object2, ax_object3, ax_object4, ax_object5, ax_object6, ax_object7, ax_object8, ax_object9, ax_object10, ax_object11, ax_object12, ax_object13, ax_object14, ax_object15, ax_object16, ax_object17, ax_object18, ax_object19, ax_object20, ax_object21, ax_object22, ax_object23, ax_object24, ax_object25, normalize=False, plot_layer='all'):
    ...
    for i,rpl in enumerate(roi_param_list):
        ...
        if custom_roilist is None:
            ...
            print('TOTAL: ' + str(len(roi_list_all) + len(roi_params['invalid_rois'])))
        ...
        for j,r in enumerate(roi_list_all):
            ...
            for tl in trialtypes:
                ...
                if valid and not roi_error:
                    ...
                    # result_max_peak['success_robustness_' + peak_trialtype].append(success_robustness)
                    # result_max_peak_ol['success_robustness_' + peak_trialtype + '_' + peak_event].append(success_robustness_ol)
    ...
    # print(roi_param_list[10],num_trialonset_long[10])
    if plot_layer is 'l5':
        num_trialonset_short = [np.mean(num_trialonset_short[0:2]),np.mean(num_trialonset_short[2:4]),np.mean(num_trialonset_short[4]),np.mean(num_trialonset_short[5]),np.mean(num_trialonset_short[6])]
        num_trialonset_long = [np.mean(num_trialonset_long[0:2]),np.mean(num_trialonset_long[2:4]),np.mean(num_trialonset_long[4]),np.mean(num_trialonset_long[5]),np.mean(num_trialonset_long[6])]
        num_lmcenter_short = [np.mean(num_lmcenter_short[0:2]),np.mean(num_lmcenter_short[2:4]),np.mean(num_lmcenter_short[4]),np.mean(num_lmcenter_short[5]),np.mean(num_lmcenter_short[6])]
        num_lmcenter_long = [np.mean(num_lmcenter_long[0:2]),np.mean(num_lmcenter_long[2:4]),np.mean(num_lmcenter_long[4]),np.mean(num_lmcenter_long[5]),np.mean(num_lmcenter_long[6])]
        num_reward_short = [np.mean(num_reward_short[0:2]),np.mean(num_reward_short[2:4]),np.mean(num_reward_short[4]),np.mean(num_reward_short[5]),np.mean(num_reward_short[6])]
        num_reward_long = [np.mean(num_reward_long[0:2]),np.mean(num_reward_long[2:4]),np.mean(num_reward_long[4]),np.mean(num_reward_long[5]),np.mean(num_reward_long[6])]

        totrois_trialonset_short = [np.sum(totrois_trialonset_short[0:2]),np.sum(totrois_trialonset_short[2:4]),np.sum(totrois_trialonset_short[4]),np.sum(totrois_trialonset_short[5]),np.sum(totrois_trialonset_short[6])]
        totrois_trialonset_long = [np.sum(totrois_trialonset_long[0:2]),np.sum(totrois_trialonset_long[2:4]),np.sum(totrois_trialonset_long[4]),np.sum(totrois_trialonset_long[5]),np.sum(totrois_trialonset_long[6])]
        totrois_lmcenter_short = [np.sum(totrois_lmcenter_short[0:2]),np.sum(totrois_lmcenter_short[2:4]),np.sum(totrois_lmcenter_short[4]),np.sum(totrois_lmcenter_short[5]),np.sum(totrois_lmcenter_short[6])]
        totrois_lmcenter_long = [np.sum(totrois_lmcenter_long[0:2]),np.sum(totrois_lmcenter_long[2:4]),np.sum(totrois_lmcenter_long[4]),np.sum(totrois_lmcenter_long[5]),np.sum(totrois_lmcenter_long[6])]
        totrois_reward_short = [np.sum(totrois_reward_short[0:2]),np.sum(totrois_reward_short[2:4]),np.sum(totrois_reward_short[4]),np.sum(totrois_reward_short[5]),np.sum(totrois_reward_short[6])]
        totrois_reward_long = [np.sum(totrois_reward_long[0:2]),np.sum(totrois_reward_long[2:4]),np.sum(totrois_reward_long[4]),np.sum(totrois_reward_long[5]),np.sum(totrois_reward_long[6])]

        tot_num_valid_rois = [np.sum(tot_num_valid_rois[0:2]),np.sum(tot_num_valid_rois[2:4]),np.sum(tot_num_valid_rois[4]),np.sum(tot_num_valid_rois[5]),np.sum(tot_num_valid_rois[6])]

        scatter_color_short = '#F970F6'
        scatter_color_long = '#FF00FF'
        fraction_plot_color_short = '#FCA4FA'
        fraction_plot_color_long = '#F98FF6'
        numbins = 30
    ...
    ax_object.set_ylim([0,0.6])
    ax_object2.set_ylim([0,0.6])
    ...
    non_nan_short = np.asarray(result_max_peak['fl_distamp_r_short'])[~np.isnan(result_max_peak['fl_distamp_p_short'])]
    non_nan_long = np.asarray(result_max_peak['fl_distamp_r_long'])[~np.isnan(result_max_peak['fl_distamp_p_long'])]
    ...
    x_pos_short = np.full(len(non_nan_short),0) + (np.random.randn(len(non_nan_short)) * 0.075)
    x_pos_long = np.full(len(non_nan_long),1) + (np.random.randn(len(non_nan_long)) * 0.075)
    ...
    non_nan_short = np.asarray(result_max_peak_ol['fl_distamp_r_short'])[~np.isnan(result_max_peak_ol['fl_distamp_p_short'])]
    non_nan_long = np.asarray(result_max_peak_ol['fl_distamp_r_long'])[~np.isnan(result_max_peak_ol['fl_distamp_p_long'])]
    parts = ax_object24.boxplot([non_nan_short, non_nan_long],
        patch_artist=True,showfliers=False,
        whiskerprops=dict(color='w', linestyle='-', linewidth=0, solid_capstyle='butt'),
        medianprops=dict(color='k', linewidth=2, solid_capstyle='butt'),
    ...
    ax_object24.set_ylabel('p value')
    ax_object24.set_xticks([0.5,3.5])
    ax_object24.set_xticklabels(['VR','DC'])

    ax_object24.set_ylim([-1.5,1.5])
    ax_object24.set_yticks([-1,0,1])
    ax_object24.set_yticklabels(['-1', '0', '1'])
...
def vr_ol_difference(max_peak_all,max_peak_all_ol,max_peak_v1,max_peak_v1_ol, subfolder, fname):
    ...
    parts = plt.boxplot([normalized_mean_ol_rsc,normalized_mean_ol_v1],patch_artist=True,
    ...
    print('mean normalized OL attenuation RSC: ', str(np.round(np.nanmean(normalized_mean_ol_rsc),4)), ' ', str(np.round(sp.stats.sem(normalized_mean_ol_rsc),4)) )
...
if __name__ == '__main__':
    ...
    ax28 = plt.subplot2grid((8,120),(4,100), rowspan=2, colspan=20)
