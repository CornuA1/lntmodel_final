...
def fig_grating_R01(h5path, sess, roi, fname, ylims=[], fformat='png', subfolder=[]):
    import numpy as np
    import h5py
    import sys
    import os
    ...
    for i in range(16):
        ...
        # plot mean trace
        ax[i].axvline(ax_mid,c='r',ls='--')
        ax[i].plot(np.nanmean(all_traces[:,i,:],1),c='k',lw=2)
        sem_dF = stats.sem(all_traces[:,i,:],1,nan_policy='omit')
        ax[i].fill_between(np.arange(len(np.nanmean(all_traces[:,i,:],1))), np.nanmean(all_traces[:,i,:],1)-sem_dF, np.nanmean(all_traces[:,i,:],1), alpha=0.25, lw=0, color='k')
        ax[i].fill_between(np.arange(len(np.nanmean(all_traces[:,i,:],1))), np.nanmean(all_traces[:,i,:],1)+sem_dF, np.nanmean(all_traces[:,i,:],1), alpha=0.25, lw=0, color='k')
    ...
    if np.size(behav_ds,1) > 10:
        ...
        ax[16].axvline(int(t_max/2),c='r',ls='--')
        ...
        ax[16].spines['bottom'].set_visible(False)
        ...
        ax[17].axvline(int(t_max/2),c='r',ls='--')
        xticks_locs = np.linspace(0,t_max,7)
