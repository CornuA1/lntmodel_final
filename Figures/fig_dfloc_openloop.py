"""
Plot trace of an individual ROI vs location in openloop condition.

Difference to the figures for the regular session is that trials where the
animal was running are separated from those where the animal was just passively
...
def fig_dfloc_openloop(h5path, sess, roi, fname, fformat='png', subfolder=[], c_ylim=[]):
    ...
    ax9.set_title('short, active')
    ax10.set_title('short, passive')
    ax11.set_title('long, active')
    ax12.set_title('long, passive')
    ...
    if np.size(behav_ds,1) > 8:

        # run through SHORT ACTIVE (animal running) trials and calculate avg dF/F for each bin and trial
        trials_active_short = filter_trials( behav_ds, [], ['animal_running',1,2], trials_short)
        mean_dF_short_succ = np.zeros((np.size(trials_active_short,0),binnr_short))
        for i,t in enumerate(trials_active_short):
            # pull out current trial and corresponding dF data and bin it
            cur_trial_loc = behav_ds[behav_ds[:,6]==t,1]
            cur_trial_dF_roi = dF_ds[behav_ds[:,6]==t,roi]
            mean_dF_trial = stats.binned_statistic(cur_trial_loc, cur_trial_dF_roi, 'mean', binnr_short,
        ...
        sem_dF_l = stats.sem(mean_dF_short_succ,0,nan_policy='omit')
        avg_mean_dF_long = np.nanmean(mean_dF_short_succ,axis=0)
        ax9.plot(avg_mean_dF_long,c=sns.xkcd_rgb["windows blue"],ls='-',lw=2) #'#00AAAA'
        # ax9.fill_between(np.arange(len(avg_mean_dF_long)), avg_mean_dF_long - sem_dF_l, avg_mean_dF_long + sem_dF_l, color = sns.xkcd_rgb["dusty purple"], alpha = 0.2)

        # run through SHORT PASSIVE trials and calculate avg dF/F for each bin and trial
        trials_passive_short = filter_trials( behav_ds, [], ['animal_notrunning',1,2], trials_short)
        mean_dF_short_unsucc = np.zeros((np.size(trials_passive_short,0),binnr_short))
        for i,t in enumerate(trials_passive_short):
            # pull out current trial and corresponding dF data and bin it
            cur_trial_loc = behav_ds[behav_ds[:,6]==t,1]
            cur_trial_dF_roi = dF_ds[behav_ds[:,6]==t,roi]
            mean_dF_trial = stats.binned_statistic(cur_trial_loc, cur_trial_dF_roi, 'mean', binnr_short,
        ...
        sem_dF_l = stats.sem(mean_dF_short_unsucc,0,nan_policy='omit')
        avg_mean_dF_long = np.nanmean(mean_dF_short_unsucc,axis=0)
        ax10.plot(avg_mean_dF_long,c=sns.xkcd_rgb["windows blue"],ls='--',lw=2)
        # ax10.fill_between(np.arange(len(avg_mean_dF_long)), avg_mean_dF_long - sem_dF_l, avg_mean_dF_long + sem_dF_l, color = sns.xkcd_rgb["dusty purple"], alpha = 0.2)

        # run through LONG ACTIVE trials and calculate avg dF/F for each bin and trial
        trials_active_long = filter_trials( behav_ds, [], ['animal_running',1,2], trials_long)
        mean_dF_long_succ = np.zeros((np.size(trials_active_long,0),binnr_long))
        for i,t in enumerate(trials_active_long):
            # pull out current trial and corresponding dF data and bin it
            cur_trial_loc = behav_ds[behav_ds[:,6]==t,1]
            cur_trial_dF_roi = dF_ds[behav_ds[:,6]==t,roi]
            mean_dF_trial = stats.binned_statistic(cur_trial_loc, cur_trial_dF_roi, 'mean', binnr_long,
        ...
        sem_dF_l = stats.sem(mean_dF_long_succ,0,nan_policy='omit')
        avg_mean_dF_long = np.nanmean(mean_dF_long_succ,axis=0)
        ax11.plot(avg_mean_dF_long,c=sns.xkcd_rgb["dusty purple"],ls='-',lw=2) #'#00AAAA'
        # ax11.fill_between(np.arange(len(avg_mean_dF_long)), avg_mean_dF_long - sem_dF_l, avg_mean_dF_long + sem_dF_l, color = sns.xkcd_rgb["dusty purple"], alpha = 0.2)

        # run through LONG PASSIVE trials and calculate avg dF/F for each bin and trial
        trials_passive_long = filter_trials( behav_ds, [], ['animal_notrunning',1,2], trials_long)
        mean_dF_long_unsucc = np.zeros((np.size(trials_passive_long,0),binnr_long))
        for i,t in enumerate(trials_passive_long):
            # pull out current trial and corresponding dF data and bin it
            cur_trial_loc = behav_ds[behav_ds[:,6]==t,1]
            cur_trial_dF_roi = dF_ds[behav_ds[:,6]==t,roi]
            mean_dF_trial = stats.binned_statistic(cur_trial_loc, cur_trial_dF_roi, 'mean', binnr_long,
        ...
        sem_dF_l = stats.sem(mean_dF_long_unsucc,0,nan_policy='omit')
        avg_mean_dF_long = np.nanmean(mean_dF_long_unsucc,axis=0)
        ax12.plot(avg_mean_dF_long,c=sns.xkcd_rgb["dusty purple"],ls='--',lw=2)
        # ax12.fill_between(np.arange(len(avg_mean_dF_long)), avg_mean_dF_long - sem_dF_l, avg_mean_dF_long + sem_dF_l, color = sns.xkcd_rgb["dusty purple"], alpha = 0.2)
    ...
    heatmap_max = np.amax([np.nanmax(np.nanmean(mean_dF_short,axis=0)),np.nanmax(np.nanmean(mean_dF_long,axis=0))])
    ...
    if np.size(behav_ds,1) > 8:
        sns.heatmap(mean_dF_short_succ,cbar=True,vmin=hmmin,vmax=hmmax,cmap='viridis',yticklabels=trials_active_short.astype('int'),xticklabels=True,ax=ax5)
        sns.heatmap(mean_dF_short_unsucc,cbar=True,vmin=hmmin,vmax=hmmax,cmap='viridis',yticklabels=trials_passive_short.astype('int'),xticklabels=True,ax=ax6)
        sns.heatmap(mean_dF_long_succ,cbar=True,vmin=hmmin,vmax=hmmax,cmap='viridis',yticklabels=trials_active_long.astype('int'),xticklabels=True,ax=ax7)
        sns.heatmap(mean_dF_long_unsucc,cbar=True,vmin=hmmin,vmax=hmmax,cmap='viridis',yticklabels=trials_passive_long.astype('int'),xticklabels=True,ax=ax8)
