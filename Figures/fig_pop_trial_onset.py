...
def fig_pop_trial_onset(behav_collection, dF_collection, rec_info, fname=[], trials='both', sortby='both', fformat='agg'):
    ...
    for i in range(len(behav_collection)):
        ...
        events = event_ind(behav_ds, ['trial_transition'])
        ...
        for roi in range(dF_ds.shape[1]):
            ...
            #normalise: first normalise mean roi sweep against entire roi signal and then normalise against the max dF or the roi within the sweep window
            roi_avg_short[:,roi] = np.mean(cur_sweep_resampled_short, axis=0)/np.max(dF_ds[:,roi])
            #roi_avg_short[:,roi] = roi_avg_short[:,roi]/np.max(roi_avg_short[:,roi])
        ...
        if sortby == 'none':
            ...
            sns.heatmap(np.transpose(roi_avg_short[:,sort_ind_short]), cmap='jet', vmin=0.0, vmax=1.0, ax=ax1, cbar=False)
            sns.heatmap(np.transpose(roi_avg_long[:,sort_ind_short]), cmap='jet', vmin=0.0, vmax=1.0, ax=ax2, cbar=False)
        elif sortby == 'long':
            sns.heatmap(np.transpose(roi_avg_short[:,sort_ind_long]), cmap='jet', vmin=0.0, vmax=1.0, ax=ax1, cbar=False)
            sns.heatmap(np.transpose(roi_avg_long[:,sort_ind_long]), cmap='jet', vmin=0.0, vmax=1.0, ax=ax2, cbar=False)
        elif sortby == 'both':
