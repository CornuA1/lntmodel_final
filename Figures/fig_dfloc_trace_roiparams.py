...
def fig_dfloc_trace_roiparams(h5path, sess, roi, fname, align_point, peak_locs, filterprops_short_1, filterprops_short_2, filterprops_long_1, filterprops_long_2, fformat='png', subfolder=[], c_ylim=[], make_figure=True, load_raw=False,use_data='aligned_data.mat', is_ol_sess=False):
    ...
    if load_raw == False:
        df_signal_path = h5path + os.sep + sess + os.sep + use_data
        dF_signal_bef =  h5py.File(df_signal_path, 'r')
        dF_ds = np.copy(dF_signal_bef['dF_aligned'])
        behav_ds = np.copy(dF_signal_bef['behaviour_aligned'])
...
#        reward_ds = np.copy(h5dat[sess + '/rewards'])
    ...
    plot_binnr_short = 340/bin_size
    plot_binnr_long = 400/bin_size
...
def run_LF180119_1_Day2018424_2():
    ...
    SESSION_OPENLOOP = 'Day2018424_openloop_4'
    ...
    roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, h5path, json_path, SUBNAME, align_point, subfolder, roi_result_params)

    if RUN_TO:
        SUBNAME = 'spaceTO'
        align_point = 'trialonset'
        subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
        roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, h5path, json_path, SUBNAME, align_point, subfolder, roi_result_params)

def run_LF180219_1_Day2018424_2():
    MOUSE = 'LF180219_1'
...
def run_LF170110_2_Day20170209_l23():
    ...
    align_point = 'landmark'
    subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
    roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, h5path, json_path, SUBNAME, align_point, subfolder, roi_result_params, False)
...
def run_LF161202_1_Day20170209_l23():
    ...
    if RUN_LM:
        ...
        align_point = 'landmark'
        subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
        roi_result_params = run_analysis(MOUSE, SESSION, '', NUM_ROIS, h5path, json_path, SUBNAME, align_point, subfolder, roi_result_params, False)
    ...
    if RUN_GOODBADTRIALS:
        subfolder = MOUSE+'_'+SESSION+'_'+'goodvbadtrials'
        run_goodvbadtrials_figure(MOUSE, SESSION, NUM_ROIS, h5path, json_path, subfolder)
...
def run_LF170613_1_Day20170804():
    ...
    NUM_ROIS = 'valid' #[41,101,13] #'all' #52
    h5path = loc_info['imaging_dir'] + MOUSE + '/' + MOUSE + '.h5'
    json_path = loc_info['figure_output_path'] + MOUSE + os.sep + MOUSE + '_' + SESSION + '.json'
    ...
    if RUN_LM:
        ...
        align_point = 'landmark'
        subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
        roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, h5path, json_path, SUBNAME, align_point, subfolder, roi_result_params, True, False)

    if RUN_TO:
        SUBNAME = 'spaceTO'
        align_point = 'trialonset'
        subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
        roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, h5path, json_path, SUBNAME, align_point, subfolder, roi_result_params, True, True)
...
def run_SIM_1_Day1_jitter():
    ...
    align_point = 'landmark'
    subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
    # roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, h5path, json_path, SUBNAME, align_point, subfolder, roi_result_params, False, False, False)
    roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, data_path, json_path, SUBNAME, align_point, subfolder, roi_result_params, True, False, True, use_data, 'jitter')

    # SUBNAME = 'spaceTO'
    # align_point = 'trialonset'
    # subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
    # roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, h5path, json_path, SUBNAME, align_point, subfolder, roi_result_params, False, True)

def run_SIM_1_Day1_wide_RF():
    MOUSE = 'SIM_1'
    SESSION = 'Day1'
    use_data = 'aligned_data.mat'
    SESSION_OPENLOOP = 'Day1_wide_RF'
    NUM_ROIS = 'all'
    json_path = loc_info['figure_output_path'] + MOUSE + os.sep + MOUSE + '_' + SESSION + '.json'
    data_path = loc_info['raw_dir'] + MOUSE
    ...
    align_point = 'landmark'
    subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
    roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, data_path, json_path, SUBNAME, align_point, subfolder, roi_result_params, True, False, True, use_data, 'wide_RF')
...
def run_LF191022_3_20191119():
    ...
    if RUN_LM:
        ...
        align_point = 'landmark'
        subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
        roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, data_path, json_path, SUBNAME, align_point, subfolder, roi_result_params, True, False, True, use_data, '')
...
def run_LF191022_1_20191115():
    ...
    align_point = 'landmark'
    subfolder = MOUSE+'_'+SESSION+'_'+SUBNAME
    roi_result_params = run_analysis(MOUSE, SESSION, SESSION_OPENLOOP, NUM_ROIS, data_path, json_path, SUBNAME, align_point, subfolder, roi_result_params, False, False, True, use_data, '')
...
def run_LF191022_1_20191207():
    MOUSE = 'LF191022_1'
    SESSION = '20191207'
    use_data = 'aligned_data.mat'
    SESSION_OPENLOOP = '20191207_ol'
    NUM_ROIS = 'valid' # 'all' #52
    # h5path = loc_info['imaging_dir'] + MOUSE + '/' + MOUSE + '.h5'
    data_path = loc_info['raw_dir'] + MOUSE
    json_path = loc_info['figure_output_path'] + MOUSE + os.sep + MOUSE + '_' + SESSION + '.json'
...
def run_LF191022_1_20191209():
    MOUSE = 'LF191022_1'
    SESSION = '20191209'
    use_data = 'aligned_data.mat'
    SESSION_OPENLOOP = '20191209_ol'
    NUM_ROIS = 'valid' # 'all' #52
    # h5path = loc_info['imaging_dir'] + MOUSE + '/' + MOUSE + '.h5'
    data_path = loc_info['raw_dir'] + MOUSE
    json_path = loc_info['figure_output_path'] + MOUSE + os.sep + MOUSE + '_' + SESSION + '.json'
...
def run_LF191023_blue_20191208():
    MOUSE = 'LF191023_blue'
    SESSION = '20191208'
    use_data = 'aligned_data.mat'
    SESSION_OPENLOOP = SESSION +'_ol'
...
def run_LF191022_3_20191113():
    MOUSE = 'LF191022_3'
    SESSION = '20191113'
    use_data = 'M01_000_000_results.mat'
    SESSION_OPENLOOP = '20191204_ol'
    NUM_ROIS = 'all' # 'all' #52
    # h5path = loc_info['imaging_dir'] + MOUSE + '/' + MOUSE + '.h5'
    data_path = loc_info['raw_dir'] + MOUSE
...
def run_LF191022_3_20191207():
    MOUSE = 'LF191022_3'
    SESSION = '20191207'
    use_data = 'M01_000_002_results.mat'
    SESSION_OPENLOOP = ''
    NUM_ROIS = 'all' # 'all' #52
    # h5path = loc_info['imaging_dir'] + MOUSE + '/' + MOUSE + '.h5'
    data_path = loc_info['raw_dir'] + MOUSE
...
def run_LF191023_blue_20191113():
    MOUSE = 'LF191023_blue'
    SESSION = '20191113'
    use_data = 'M01_000_003_results.mat'
    SESSION_OPENLOOP = ''
    NUM_ROIS = 'all' # 'all' #52
    # h5path = loc_info['imaging_dir'] + MOUSE + '/' + MOUSE + '.h5'
    data_path = loc_info['raw_dir'] + MOUSE
...
def run_LF191023_blue_20191208_n():
    MOUSE = 'LF191023_blue'
    SESSION = '20191208'
    use_data = 'M01_000_002_results.mat'
    SESSION_OPENLOOP = ''
...
def run_through_some_shit():
    MOUSE = 'LF191022_1'
    SESSION = '20191213'
    use_data = 'M01_000_004_results.mat'
    SESSION_OPENLOOP = ''
    NUM_ROIS = 'all' # 'all' #52
    # h5path = loc_info['imaging_dir'] + MOUSE + '/' + MOUSE + '.h5'
    data_path = loc_info['raw_dir'] + MOUSE
...
#    run_LF191022_3_20191207()
#    run_LF191022_3_20191113() 
#    run_LF191022_1_20191209_n()
#    run_LF191022_1_20191114()
#    run_LF191023_blue_20191113()
#    run_LF191023_blue_20191208_n()
    # run_through_some_shit()
