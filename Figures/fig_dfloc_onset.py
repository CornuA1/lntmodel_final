...
def fig_dfloc_onset(h5path, sess, roi, fname, fformat='png', subfolder=[], c_ylim=[]):
    ...
    for i,t in enumerate(trials_short):
        ...
        # remove first and last 5 sample points as alignments can lead to interpolation artifacts
        cur_trial_loc = cur_trial_loc[5:-5]
        cur_trial_dF_roi = cur_trial_dF_roi[5:-5]
        # align all trials to 0 by subtracting the random distance they were placed into the track
    ...
    # plot heatmaps
    sns.heatmap(mean_dF_short,cbar=True,vmin=hmmin,vmax=hmmax,cmap='jet',yticklabels=trials_short.astype('int'),xticklabels=True,ax=ax3)
    sns.heatmap(mean_dF_long,cbar=True,vmin=hmmin,vmax=hmmax,cmap='jet',yticklabels=False,xticklabels=False,ax=ax4)

    sns.heatmap(mean_dF_short_succ,cbar=False,vmin=hmmin,vmax=hmmax,cmap='jet',yticklabels=trials_short_succ.astype('int'),xticklabels=True,ax=ax5)
    sns.heatmap(mean_dF_short_unsucc,cbar=False,vmin=hmmin,vmax=hmmax,cmap='jet',yticklabels=trials_short_unsucc.astype('int'),xticklabels=True,ax=ax6)
    sns.heatmap(mean_dF_long_succ,cbar=False,vmin=hmmin,vmax=hmmax,cmap='jet',yticklabels=trials_long_succ.astype('int'),xticklabels=True,ax=ax7)
    sns.heatmap(mean_dF_long_unsucc,cbar=False,vmin=hmmin,vmax=hmmax,cmap='jet',yticklabels=trials_long_unsucc.astype('int'),xticklabels=True,ax=ax8)

    if c_ylim == []:
        ax1.set_ylim([0,max_y])
        ax2.set_ylim([0,max_y])
        ax9.set_ylim([0,max_y])
        ax10.set_ylim([0,max_y])
