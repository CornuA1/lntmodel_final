...
def fig_popvloc_s5(included_recordings, rois='task_engaged_all', trials='both', sortby='both', fformat='png', fname='', subfolder=''):
    ...
    for current_iter in range(NUM_ITER):
        tot_rois = 0
        mean_dF_short_coll_sig_vr = []
        mean_dF_short_coll_sig_ol = []
        mean_dF_long_coll_sig_vr = []
        mean_dF_long_coll_sig_ol = []
        ...
        for r in included_recordings:
            ...
            behav_ds_ol = np.copy(h5dat[session + '_openloop' + '/behaviour_aligned'])
            dF_ds_ol = np.copy(h5dat[session + '_openloop' + '/dF_win'])
            h5dat.close()
            ...
            trials_short_sig_vr = filter_trials(behav_ds, [], ['tracknumber',track_short])
            trials_long_sig_vr = filter_trials(behav_ds, [], ['tracknumber',track_long])
            trials_short_sig_ol = filter_trials(behav_ds_ol, [], ['tracknumber',track_short])
            trials_long_sig_ol = filter_trials(behav_ds_ol, [], ['tracknumber',track_long])

            # randomly draw 50% of indices to calculate signal and the other half to calculate the order by which to sort
            # trials_short_rand_vr = np.random.choice(trials_short_sig_vr, np.size(trials_short_sig_vr), replace=False)
            # trials_short_rand_ol = np.random.choice(trials_short_sig_ol, np.size(trials_short_sig_ol), replace=False)
            # trials_short_sig_vr = trials_short_rand_vr[np.arange(np.ceil(np.size(trials_short_sig_vr)/2),np.size(trials_short_sig_vr)).astype(int)]
            # trials_short_sig_ol = trials_short_rand[np.arange(np.ceil(np.size(trials_short_sig_ol)/2),np.size(trials_short_sig_ol)).astype(int)]
            #
            # trials_long_rand_vr = np.random.choice(trials_long_sig_vr, np.size(trials_long_sig_vr), replace=False)
            # trials_long_rand_ol = np.random.choice(trials_long_sig_ol, np.size(trials_long_sig_ol), replace=False)
            # trials_long_sig_vr = trials_long_rand_vr[np.arange(np.ceil(np.size(trials_long_sig_vr)/2),np.size(trials_long_sig_vr)).astype(int)]
            # trials_long_sig_ol = trials_long_rand_ol[np.arange(np.ceil(np.size(trials_long_sig_ol)/2),np.size(trials_long_sig_ol)).astype(int)]
            ...
            mean_dF_short_sig_vr = np.zeros((binnr_short,np.size(roi_selection)))
            mean_dF_short_sig_ol = np.zeros((binnr_short,np.size(roi_selection)))

            mean_dF_long_sig_vr = np.zeros((binnr_long,np.size(roi_selection)))
            mean_dF_long_sig_ol = np.zeros((binnr_long,np.size(roi_selection)))

            for j,roi in enumerate(roi_selection):
                # intilize matrix to hold data for all trials of this roi
                mean_dF_trials_short_sig_vr = np.zeros((np.size(trials_short_sig_vr,0),binnr_short))
                mean_dF_trials_short_sig_ol = np.zeros((np.size(trials_short_sig_ol,0),binnr_short))

                # calculate mean dF vs location for each ROI on short trials
                for k,t in enumerate(trials_short_sig_vr):
                    ...
                    mean_dF_trials_short_sig_vr[k,:] = mean_dF_trial

                #mean_dF_short_sig[:,j] = np.nanmean(mean_dF_trials_short_sig,0)
                mean_dF = np.nanmean(mean_dF_trials_short_sig_vr,0)
                mean_dF_short_sig_vr[:,j] = (mean_dF - np.nanmin(mean_dF))/(np.nanmax(mean_dF) - np.nanmin(mean_dF))
                mean_dF_short_sig_vr[:,j] = mean_dF
                ...
                # calculate mean dF vs location for each ROI on short trials
                for k,t in enumerate(trials_short_sig_ol):
                    # pull out current trial and corresponding dF data and bin it
                    cur_trial_loc = behav_ds_ol[behav_ds_ol[:,6]==t,1]
                    cur_trial_rew_loc = behav_ds_ol[behav_ds_ol[:,6]==t+1,1]
                    # if the location was reset to 0 as the animal got a reward (which is the case when it gets a default reward: add the last location on the track to blackbox location)
                    if np.size(cur_trial_rew_loc) > 0:
                        if cur_trial_rew_loc[0] < 100:
                            cur_trial_rew_loc += cur_trial_loc[-1]
                        cur_trial_loc = np.append(cur_trial_loc, cur_trial_rew_loc)
                    cur_trial_dF_roi = dF_ds_ol[behav_ds_ol[:,6]==t,roi]
                    if np.size(cur_trial_rew_loc) > 0:
                        cur_trial_dF_roi = np.append(cur_trial_dF_roi, dF_ds_ol[behav_ds_ol[:,6]==t+1,1])
                    mean_dF_trial = stats.binned_statistic(cur_trial_loc, cur_trial_dF_roi, 'mean', bin_edges_short, (track_start, tracklength_short))[0]
                    mean_dF_trials_short_sig_ol[k,:] = mean_dF_trial

                #mean_dF_short_sig[:,j] = np.nanmean(mean_dF_trials_short_sig,0)
                mean_dF = np.nanmean(mean_dF_trials_short_sig_ol,0)
                mean_dF_short_sig_ol[:,j] = (mean_dF - np.nanmin(mean_dF))/(np.nanmax(mean_dF) - np.nanmin(mean_dF))
                mean_dF_short_sig_ol[:,j] = mean_dF

                # calculate mean dF vs location for each ROI on long trials
                mean_dF_trials_long_sig_vr = np.zeros((np.size(trials_long_sig_vr,0),binnr_long))
                mean_dF_trials_long_sig_ol = np.zeros((np.size(trials_long_sig_ol,0),binnr_long))

                # calculate mean dF vs location for each ROI on long trials
                for k,t in enumerate(trials_long_sig_vr):
                    # pull out current trial and corresponding dF data and bin it
                    cur_trial_loc = behav_ds[behav_ds[:,6]==t,1]
                    cur_trial_rew_loc = behav_ds[behav_ds[:,6]==t+1,1]
                    # if the location was reset to 0 as the animal got a reward (which is the case when it gets a default reward: add the last location on the track to blackbox location)
                    ...
                    mean_dF_trials_long_sig_vr[k,:] = mean_dF_trial

                #mean_dF_long_sig[:,j] = np.nanmean(mean_dF_trials_long_sig,0)
                mean_dF = np.nanmean(mean_dF_trials_long_sig_vr,0)
                mean_dF_long_sig_vr[:,j] = (mean_dF - np.nanmin(mean_dF))/(np.nanmax(mean_dF) - np.nanmin(mean_dF))
                mean_dF_long_sig_vr[:,j] = mean_dF
                # print(np.nanmin(mean_dF), np.nanmax(mean_dF))

                #mean_dF_long_sig[:,j] /= np.nanmax(np.abs(mean_dF_long_sig[start_bin:end_bin_long,j]))

                # calculate mean dF vs location for each ROI on long trials
                for k,t in enumerate(trials_long_sig_ol):
                    # pull out current trial and corresponding dF data and bin it
                    cur_trial_loc = behav_ds_ol[behav_ds_ol[:,6]==t,1]
                    cur_trial_rew_loc = behav_ds_ol[behav_ds_ol[:,6]==t+1,1]
                    # if the location was reset to 0 as the animal got a reward (which is the case when it gets a default reward: add the last location on the track to blackbox location)
                    ...
                    if np.size(cur_trial_rew_loc) > 0:
                        cur_trial_dF_roi = np.append(cur_trial_dF_roi, dF_ds_ol[behav_ds_ol[:,6]==t+1,1])
                    mean_dF_trial = stats.binned_statistic(cur_trial_loc, cur_trial_dF_roi, 'mean', bin_edges_long, (track_start, tracklength_long))[0]
                    mean_dF_trials_long_sig_ol[k,:] = mean_dF_trial

                #mean_dF_long_sig[:,j] = np.nanmean(mean_dF_trials_long_sig,0)
                mean_dF = np.nanmean(mean_dF_trials_long_sig_ol,0)
                mean_dF_long_sig_ol[:,j] = (mean_dF - np.nanmin(mean_dF))/(np.nanmax(mean_dF) - np.nanmin(mean_dF))
                mean_dF_long_sig_ol[:,j] = mean_dF

            if mean_dF_short_coll_sig_vr == []:
                mean_dF_short_coll_sig_vr = mean_dF_short_sig_vr
            else:
                mean_dF_short_coll_sig_vr = np.append(mean_dF_short_coll_sig_vr, mean_dF_short_sig_vr, axis=1)

            if mean_dF_short_coll_sig_ol == []:
                mean_dF_short_coll_sig_ol = mean_dF_short_sig_ol
            else:
                mean_dF_short_coll_sig_ol = np.append(mean_dF_short_coll_sig_ol, mean_dF_short_sig_ol, axis=1)

            if mean_dF_long_coll_sig_vr == []:
                mean_dF_long_coll_sig_vr = mean_dF_long_sig_vr
            else:
                mean_dF_long_coll_sig_vr = np.append(mean_dF_long_coll_sig_vr, mean_dF_long_sig_vr, axis=1)

            if mean_dF_long_coll_sig_ol == []:
                mean_dF_long_coll_sig_ol = mean_dF_long_sig_ol
            else:
                mean_dF_long_coll_sig_ol = np.append(mean_dF_long_coll_sig_ol, mean_dF_long_sig_ol, axis=1)
        ...
        for row in range(popvec_cc_matrix_short_size):
            for col in range(popvec_cc_matrix_short_size):
                ...
                popvec_cc_matrix_ss_pearsonr[row,col,current_iter] = stats.pearsonr(mean_dF_short_coll_sig_vr[row,:],mean_dF_short_coll_sig_ol[col,:])[0]
                popvec_cc_matrix_sl_pearsonr[row,col,current_iter] = stats.pearsonr(mean_dF_short_coll_sig_ol[row,:],mean_dF_long_coll_sig_ol[col,:])[0]
        ...
        for row in range(popvec_cc_matrix_long_size):
            for col in range(popvec_cc_matrix_long_size):
                ...
                popvec_cc_matrix_ll_pearsonr[row,col,current_iter] = stats.pearsonr(mean_dF_long_coll_sig_vr[row,:],mean_dF_long_coll_sig_ol[col,:])[0]
    ...
    ax7_img = ax7.pcolormesh(popvec_cc_matrix_sl_mean.T,vmin=0.0,vmax=1.0,cmap='viridis')
    ...
    ax5_img = ax5.pcolor(popvec_cc_matrix_ss_mean.T,vmin=0.0,vmax=1.0,cmap='viridis')
    ...
    ax6_img = ax6.pcolormesh(popvec_cc_matrix_ll_mean.T,vmin=0.0,vmax=1.0,cmap='viridis')
    ...
    mean_dF_vr_short = np.zeros(mean_dF_short_coll_sig_vr.shape[1])
    for i, row in enumerate(mean_dF_short_coll_sig_vr.T):
        if not np.all(np.isnan(row)):
            mean_dF_vr_short[i] = np.nanargmax(row)
        else:
            print('WARNING: sort signal with all NaN encountered. ROI not plotted.')
            pass
    sort_ind_short = np.argsort(mean_dF_vr_short)
...
#    sns.heatmap(np.transpose(mean_dF_short_coll[start_bin:end_bin_short, sort_ind_short]), cmap='jet', vmin=0.0, vmax=1.0, ax=ax1, cbar=False)
    ...
    mean_dF_vr_long = np.zeros(mean_dF_long_coll_sig_vr.shape[1])
    for i, row in enumerate(mean_dF_long_coll_sig_vr.T):
        if not np.all(np.isnan(row)):
            mean_dF_vr_long[i] = np.nanargmax(row)
        else:
            print('WARNING: sort signal with all NaN encountered. ROI not plotted.')
            pass
    sort_ind_long = np.argsort(mean_dF_vr_long)
...
if __name__ == "__main__":
    ...
    fig_popvloc_s5(figure_datasets, 'task_engaged_all', 'both', 'both', 'png', 'task_engaged_all_vrol', 'popvloc')
    # # #
    # # # # #
    ...
    # fig_popvloc_s5(figure_datasets, 'task_engaged_all', 'both', 'both', 'png', 'task_engaged_V1_vrol', 'popvloc')
